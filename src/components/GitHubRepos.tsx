...
export default async function GitHubRepos() {
  ...
  return (
    <div className="bento bento-3">
      {filtered.slice(0, 9).map((repo: GitHubRepo) => (
        <a
          key={repo.id}
          href={repo.html_url}
          target="_blank"
          rel="noopener noreferrer"
          className="glass p-4 group block"
        >
          <div className="flex items-start justify-between gap-2 mb-1.5">
            <h3 className="text-sm font-medium text-[var(--text-primary)] group-hover:text-[var(--accent)] transition-colors truncate">
              {repo.name}
            </h3>
            <svg className="w-3 h-3 text-[var(--text-tertiary)] group-hover:text-[var(--accent)] transition-colors flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
